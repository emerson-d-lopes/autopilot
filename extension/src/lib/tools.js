...
async function resolvePoint(tabId, { ref, coordinate }, { requireHit = false, paymentCategory } = {}) {
  ...
  if (!coordinate) throw new ToolError('invalid_argument', 'this action needs either coordinate [x, y] or ref');
...
async function computerTool(ctx, input) {
  ...
  if (!action) throw new ToolError('invalid_argument', 'computer requires an action');
  if (tabId === undefined || tabId === null) throw new ToolError('invalid_argument', 'computer requires a tabId');
  ...
  switch (action) {
    ...
    case 'zoom': {
      if (!input.region || input.region.length !== 4) {
        throw new ToolError('invalid_argument', 'zoom requires region [x0, y0, x1, y1]');
      }
      const paint = await waitForPaint(tabId);
      await pageCall(tabId, { type: 'HIDE_FOR_TOOL_USE' }).catch(() => {});
      try {
    ...
    case 'scroll_to': {
      if (!input.ref) throw new ToolError('invalid_argument', 'scroll_to requires ref');
    ...
    case 'left_click_drag': {
      ...
      if (!input.start_coordinate || !input.coordinate) {
        throw new ToolError('invalid_argument', 'left_click_drag requires start_coordinate and coordinate');
      }
    ...
    case 'type': {
      if (input.text === undefined || input.text === null) {
        throw new ToolError('invalid_argument', 'type requires text');
      }
      await perms.verifyOriginUnchanged(tabId, url);
      if (input.ref) {
    ...
    case 'key': {
      if (!input.text) throw new ToolError('invalid_argument', 'key requires text, e.g. "Enter" or "ctrl+a"');
      await perms.verifyOriginUnchanged(tabId, url);
    ...
    default:
      throw new ToolError('invalid_argument', 'unknown computer action: ' + action);
  }
}
...
export const handlers = {
  ...
  find: async (ctx, input) => {
    ...
    if (!input.query) throw new ToolError('invalid_argument', 'find requires a query');
    const paymentCategory = await isPaymentCategory(url);
  ...
  file_upload: async (ctx, input) => {
    ...
    if (!ref && !coordinate) throw new ToolError('invalid_argument', 'file_upload requires a ref or a coordinate');
    if (!Array.isArray(paths) || !paths.length) {
      throw new ToolError('invalid_argument', 'file_upload requires a non-empty paths array');
    }
    ...
    try {
      if (marked.isFileInput) {
        if (paths.length > 1 && !marked.multiple) {
          throw new ToolError('invalid_argument', 'this input accepts one file, ' + paths.length + ' were given');
        }
...
export async function execute(name, input, ctx) {
  ...
  if (!handler) throw new ToolError('invalid_argument', 'unknown tool: ' + name);
  try {
    return await handler(ctx, input || {});
  } catch (err) {
    if (err instanceof perms.PermissionDenied) throw err;
    throw withCode(err, 'unknown_failure', { effects: 'unknown' });
  }
}
