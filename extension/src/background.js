...
async function handleMessage(message) {
  ...
  switch (message.type) {
    ...
    case 'tool_request': {
      ...
      // The host stamps a correlation id on the envelope. It travels back on
      // every response, including a failure, so one line in the journal, one
      // result and one error all name the same call.
      const correlationId = message.correlationId ?? message.callId ?? toolUseId ?? id;
      const bornAt = generation;
      const { name: tool, input: args } = normalizeCall(message.tool, message.args);
      const ctx = { clientId: clientId || 'default', toolUseId, correlationId };
      ...
      try {
        ...
        respond({ type: 'tool_response', id, correlationId, result: { ...result, id: correlationId }, tab }, bornAt, tool);
      } catch (err) {
        ...
        respond({ type: 'tool_response', id, correlationId, error, tab }, bornAt, tool);
      } finally {
        inFlight--;
      }
      return;
